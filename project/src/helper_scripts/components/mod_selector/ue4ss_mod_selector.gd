...
func _read_active_mods_from_fs() -> Array:
  var base_dir := UE4SSHelper.get_mods_dir()
  ...
  var manageable_mods := UE4SSHelper.get_mods_in_mods_file().filter(func(mod: Array) -> bool:
    return mod[UE4SSHelper.MOD_ACTIVATED] and mod[UE4SSHelper.MOD_NAME] not in _unmanageable_mods_cache and mod[UE4SSHelper.MOD_NAME] not in _PREINSTALLED_MODS
  ).map(func(mod: Array) -> String:
    return mod[UE4SSHelper.MOD_NAME]
  )

  return _unmanageable_mods_cache + manageable_mods

func _read_other_available_mods_from_fs() -> Array:
  return FileSystem.directories_in(UE4SSHelper.get_mods_dir()).filter(func(dir: String) -> bool:
...
func _custom_post_save_actions() -> void:
  ...
  FileSystem.write_lines(UE4SSHelper.get_mods_file(), mods)

func _other_available_mod_is_not_found(mod: String) -> bool:
  return not FileSystem.is_dir(UE4SSHelper.get_active_mod_dir(mod))
...
func _persist_mod_file_addition(mod: String, file: String, add_mode: AddMode.Value) -> void:
  if add_mode == AddMode.MOVE_ON_ADD:
    _persist_mod_dir_move(_get_mod_dir_from_file(file), UE4SSHelper.get_active_mod_dir(mod))
...
func _get_active_paths_for_mod(mod: String) -> Array:
  var mod_dir := UE4SSHelper.get_active_mod_dir(mod)
...
func _convert_active_paths_to_available(mod: String, active_paths: Array, status: ModStatus.Value) -> Array:
  if status == ModStatus.REGULAR:
    ...
    var active_dir := UE4SSHelper.get_active_mod_dir(mod)
